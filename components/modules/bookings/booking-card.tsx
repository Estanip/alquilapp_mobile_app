    ...
    <View style={{ alignItems: 'center' }} key={_booking_id}>
      <Card style={tailwind`mt-2 w-90`}>
        <CardContent style={tailwind`gap-1`}>
          {_infoDetails?.map(({ subtitle, text, playersText }: IBookingDetails, index) => (
            <View key={`players-view-${index}`}>
              {subtitle === BookingDetailsSubtitles.PLAYERS ? (
                <View style={{ display: 'flex', flexDirection: 'column' }}>
                  <CardSubtitle style={tailwind`uppercase pb-2`}>{subtitle}:</CardSubtitle>
                  {playersText?.map((e, i) => (
                    <View
                      key={`subtitle-${i}`}
                      style={{
                    ...
                    >
                      <CardText style={tailwind`capitalize font-semibold`}>{e.name} </CardText>
                ...
                >
                  {subtitle === BookingDetailsSubtitles.PRICE ? (
                    <>
                      ...
                      >
                        <CardSubtitle style={tailwind`uppercase`}>{subtitle}:</CardSubtitle>
                        <Text> </Text>
          ...
          >
            ...
            {_cancelButton && _status === BookingStatus.ACTIVE && _validateLimitDate() ? (
              <CommonButton
