...
export default function BookScreen(): React.JSX.Element {
  ...
  const [courtsFieldsData, setCourtsFieldsData] = useState<IField[]>(initialDataState.courtNumbers);
  ...
  const _getAvailbleSchedules = async (date: Date, reservationFrom?: string) => {
    ...
    if (date && courtNumber) {
      ...
      if (selectedCourt) {
        ...
        const courtAvailabilityFrom = parseInt(selectedCourt?.available_from?.substring(0, 2), 10);
        const courtAvailabilityTo = parseInt(selectedCourt?.available_until?.substring(0, 2), 10);
  ...
  const _filterPlayersSelection = (word: string) => {
    ...
    else if (word?.length > 2) setPlayersFieldsData(playersFieldsData.filter(e => e.name === word));
  };
  ...
  const onChangeCheck = (isChecked: boolean) => {
    ...
    if (isChecked && players.length < 4 && user_id) {
      ...
      if (currentPlayers.length >= 2 && currentPlayers.length <= 4)
        setSteps({ ...steps, _players: StepStatus.DONE });
      else if (currentPlayers.length < 2) setSteps({ ...steps, _players: StepStatus.PENDING });
    }
    setIsLoading(false);
  };
  ...
  const onChangePlayers = (selectedPlayers: any, isEditing = false) => {
    ...
    if (selectedPlayers.length === 5 && players.length === 4)
      ...
      if (selectedPlayers.length >= 2 && selectedPlayers.length <= 4)
        setSteps({ ...steps, _players: StepStatus.DONE });
      else if (selectedPlayers.length < 2) setSteps({ ...steps, _players: StepStatus.PENDING });
        ...
        <View key="View-container" style={bookStyles.view}>
          ...
          {steps._players === StepStatus.DONE && (
            <CommonButton
              ...
              _buttonText={route.params?._id ? ButtonTextActions.EDIT : ButtonTextActions.BOOK}
              _onClick={() =>
                _confirmBooking(
                  Number(courtNumber!),
                  date!,
