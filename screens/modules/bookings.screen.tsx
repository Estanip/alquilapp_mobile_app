...
export default function BookingsScreen(): React.JSX.Element {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = (await BookingsService().getByOwner(token!, user_id!)) as TReservations;
  ...
  const _cancelBooking = async (booking_id: string) => {
    const _remove = async (token: string, id: string) => {
      ...
      if (response?.statusCode !== 200) return showErrorAlert('No se pudo cancelar la reserva');
    };
                      ...
                      [
                        {
                          ...
                          text: _formatDateToString(reservation.date as string),
                        },
                        {
                    ...
                    _infoDetails={
                      [
                        ...
                        },
                        {
                          ...
                          playersText: reservation.players.length
                            ? reservation.players.map(
                                ({ fee, first_name, last_name, membership_type }) => {
                                  return {
                                    name: `${first_name} ${last_name}`,
                                    membership: membership_type,
                                    fee,
