...
export default function LoginScreen(): React.JSX.Element {
  ...
  const confirmLogin = async () => {
    if (password === '' || email === '' || !validatedData) showAlert('Error', 'Datos incompletos');
    else {
      ...
      if (!result) showAlert('Error', 'Estamos teniendo inconvenientes, intente más tarde');
        ...
        signIn(result?.data.token, result?.data._id, result.data.is_enabled, result.data.email);
        if (!result.data?.is_enabled) router.replace({ pathname: routes.CODE_VERIFICATION });
